...
const DatabaseService = {
    ...
    puppet_courses: {
        async findByName(courseName) {
            try {
                console.log('查询课程, 名称:', courseName);
                const Course = new BaaS.TableObject(DatabaseService.tables.puppet_courses);
                const query = new BaaS.Query();
                query.compare('course_name', '=', courseName);
                const result = await Course.setQuery(query).find();
                console.log('查询结果:', result);
                if (result.data.objects.length > 0) {
                    console.log('课程数据:', result.data.objects[0]);
